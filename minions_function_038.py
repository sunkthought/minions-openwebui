...
def _fix_json_escape_sequences(json_string: str) -> str:
    ...
    # Common pattern: \a, \b (not backspace), \f (not form feed), etc.
    invalid_escapes = [r'\\a', r'\\c', r'\\d', r'\\e', r'\\g', r'\\h', r'\\i', r'\\j', r'\\k', r'\\l', r'\\m', r'\\o', r'\\p', r'\\q', r'\\s', r'\\w', r'\\x', r'\\y', r'\\z']
    
    for escape in invalid_escapes:
        # Replace invalid escapes with just the character (remove backslash)
        json_string = re.sub(escape, escape[1:], json_string, flags=re.IGNORECASE)
    
    # Fix double backslashes that should be single
    json_string = re.sub(r'\\\\(?!["\\/bfnrt])', r'\\', json_string)
    
    # Fix unescaped quotes in string values
    # This is a simple fix - in practice you might need more sophisticated handling
...
class Pipe:
    ...
    def __init__(self):
        ...
        self.name = "MinionS v0.3.8 (Decomposition)"
